...
- [x] A screen consuming the paged user stream: `collectAsLazyPagingItems()` with a load-state footer, search pushed into `UserPagingDao.pagingSource()`, and a source for `RefreshVisibleUsersUseCase`'s visible ids that survives the move — added 2026-09-13 while closing the Phase 10 `LazyColumn` item, which `docs/paging.md` had expected to carry this. It cannot: an in-memory filter over a `PagingData` sees only the pages already loaded, so search becomes a `LIKE` in the DAO and a change to the `PagedUserRepository` contract; and the refresh fan-out is called with the ids on screen, which under paging live in the presenter rather than in `HomeViewModel`. Both are contract decisions in the data layer, which is why they are here and not there — see `docs/lazy-lists.md` — done: `HomeScreen` renders the stream, `HomeContent` is deleted in favour of `LazyPagingItems.loadState`, and the refresh reads `LazyListState.layoutInfo` at the tap. The third decision was not in the item: a search also turns the `RemoteMediator` off, because a filtered `PagingSource` that keeps appending walks the entire remote list on a keystroke — so a search covers what has been downloaded, and the screen says so (PR #48)

## Phase 10 — Compose Performance & UI
- [x] Recomposition profiling: Layout Inspector counts + a fix for an unstable-lambda hotspot — the pass found no hotspot, and that finding is what the change is for. `onClick = { viewModel.onEvent(Clicked) }`, the shape all six screens were written in, is the textbook unstable capture, and whether it costs a recomposition is decided entirely by **strong skipping**: with it off the lambda is not memoised, so it is a new instance every recomposition and nothing below the screen can skip; with it on the lambda *is* memoised, keyed on the view model compared by identity, and since `hiltViewModel()` returns the same instance across recompositions the same lambda comes back. This build compiles with it on — Kotlin 2.1.0, no `composeCompiler { }` block anywhere, so the plugin default applies. So the screens were not slow; they were getting a property their own source did not have from a compiler default, one that was opt-in as recently as Kotlin 2.0.0 and that a single line still turns off. Nothing failed while that held and nothing would have failed when it stopped — it would have surfaced as jank months later, in a search field nobody had touched, which is the same silent-and-expensive-later failure `StabilityContractTest` exists for. `rememberEventSink` returns `onEvent` as one remembered `(E) -> Unit`; function types are in the compiler's known-stable set, so the callbacks built from it capture only stable values and are memoised under either mode, and the skipping becomes a property of the source. `RecompositionContractTest` reads *source* rather than compiled output, deliberately: whether a lambda was memoised is not recorded in the class file as a fact about the lambda, because Kotlin 2.x compiles lambdas through `invokedynamic` and what is left is a synthetic method whose parameters happen to be the captures — a check on that shape would be a check on a calling convention and would go quiet, passing and auditing nothing, the first time the convention changed. Not done, and the reason the counts in `docs/recomposition.md` are a procedure rather than a table: **nothing was measured on a device**. The agent's environment has no Android SDK, no emulator and no Android Studio — `dl.google.com` answers 403 on CONNECT — so Layout Inspector, `--metricsDestination` and Macrobenchmark can none of them run here; the doc separates what is procedure from what is derived from the compiler's documented memoisation rules, and names the one experiment most likely to contradict the analysis: the navigation rail under `MainNavScaffold`, which is a scope-hoisting question and belongs to the `derivedStateOf` item below. Also not done: the contract test covers the view model specifically, not every unstable capture — `AppNavHost`'s navigation callbacks close over `NavHostController` and are in exactly the same position, with no equivalent of an event sink to hoist them to (PR #44)
- [x] Stability: `@Immutable`, `@Stable`, and the compiler-metrics report checked into CI — the annotation half was already done and checked by `StabilityContractTest` in PR #29, so what this item was actually missing was the report. `configureComposeCompilerReports` sets `metricsDestination`/`reportsDestination` behind `-PcomposeCompilerReports=true`, CI sets it for the whole workflow as `ORG_GRADLE_PROJECT_composeCompilerReports`, and `scripts/verify-compose-metrics.py` reads what the compiler wrote. `--no-build-cache` on the compile step is load-bearing: the destinations are `FilesSubpluginOption`s of the default (INTERNAL) kind, so they are not declared task outputs and a compile restored from the cache writes no report at all — the same shape as the `copyRoomSchemas` NO-SOURCE problem in `docs/room-migrations.md`. Three things this run learned the hard way. **One**: the two report files count different things. `-composables.txt` names top-level composable functions; `-module.json` counts every composable including lambdas, which have no name to print — 45 against 164 here, and `:app` is 2 against 13 (`AppNavHost` and `MainNavScaffold`, everything else being its nav graph's lambdas). An equality between them looks exactly like the check this gate should have and would fail permanently against a correct parser, so it is written down as a *non*-check next to the real ones. **Two**: the first version shipped green while dropping parameters. A parameter's default value is printed as the compiler's own lowered IR and runs for a dozen lines at any indentation, column zero included, so the `)` closing `rememberNavController(` read as the end of `AppNavHost` and took `startDestination` — an unstable parameter, the entire thing this gate looks for — with it, silently. The parser tracks bracket depth now, and `assert_parser_understood_the_report` fails on anything in declaration position it cannot read, because the output of this gate is "nothing to report" and every way of reading less produces exactly that. **Three**: with strong skipping on, every named composable in the app is already skippable — `AppNavHost` takes three unstable parameters and is skippable anyway, since strong skipping compares them by identity. So a green run means "nothing recomposes unconditionally", not "nothing is unstable"; the summary carries an `unstable params` column (12 across the app) because that is the number that does not go quiet, and the skippability rule earns its place mainly on the day a `composeCompiler { }` line turns strong skipping off. Not done: composable **lambdas** are counted by the compiler and not named in the report, so nothing gates on them; no aggregate baseline, which is the only thing that would put them under a budget; debug variant only. None of the six Gradle gates ran locally — `dl.google.com` answers 403 on CONNECT from the agent's environment — so the 21-scenario self-test (three of its fixtures copied from the real reports CI produced) and the offline JVM harness are what was verified before each push (PR #45)
- [x] `derivedStateOf` and `remember` keying to cut redundant work — two changes and three non-changes, and the three are the more useful half, because "wrap it in `derivedStateOf`" is the reflex and in each of them it buys nothing. A derivation pays under one condition — the source changes more often than the result, **and** the reading scope is not already reading that source for something else — and exactly one site in the app meets both halves. `HomeUiState.searchQuery` is bound to the text field undebounced on purpose, so `state` is a new object per keystroke; the app bar's `actions` slot is a composable lambda with its own scope and wants one `Boolean` out of it, so reading `state.isRefreshing` there made the app bar a reader of the search field. `remember { derivedStateOf { … } }` removes the invalidation rather than the render, and the change is small — what is not small is the reason the other three were left alone. `state.searchQuery` in the Scaffold body is the anti-pattern in its purest form (a derivation that changes exactly as often as its source *is* its source, plus an allocation and a dependency record), and the body is invalidated by `SearchBar`'s own read whatever the other two fields are wrapped in. The flash action on both camera screens reads both fields of a two-field state, so there is nothing to narrow — and nothing arriving at speed either, because `_state.update { … }` returns the same instance when a frame lands on a result already on screen and `MutableStateFlow` conflates it, so ML Kit's several-frames-a-second never reaches the composition. `useListDetailLayout()` is the textbook shape — window metrics update continuously through a drag-resize, the `Boolean` changes at one boundary — and the one case `derivedStateOf` *cannot* address: a derivation narrows a read it performs inside its own block, a block is not a composable context, and by the time a `@Composable` accessor has returned a value its state reads are already recorded against the caller. Narrowing that needs an API handing back a `State`, which `currentWindowAdaptiveInfo()` is not. The `remember` half is `MainNavScaffold`, and the cost there is not the three allocations: `items` is the parameter `AdaptiveNavigationScaffold` skips on, `ImmutableList` puts it on structural equality, and `ImageVector.equals` compares the whole path tree — so a structurally identical rebuild paid a deep comparison of two Material icons per item, every pass, to conclude nothing had changed. How often "every pass" is, is not up to that function: `useListDetailLayout()` returns a value, so it is not restartable, so its window-metrics read lands on the `Home` entry's scope and every posture or size update re-invokes the scaffold. `DerivedStateContractTest` holds the two rules whose broken forms compile, read and behave identically until they do not — a `derivedStateOf` outside a `remember` keeps neither its recorded dependencies nor its previous result, which are its only two reasons to exist, and an unkeyed `remember` over a parameter renders the argument the screen was first given for as long as it stays composed — with the state factories exempt by name, because `mutableStateOf` takes an *initial* value by definition and keying it would discard whatever the user had done. Both call-site sets are pinned, because whether a derivation narrows is a claim about runtime frequencies that no source check can settle; the pin puts a reviewer in front of the claim and `docs/derived-state.md` is where it goes. `KotlinSource` moved to `SourceTree.kt` rather than being copied and gained character-literal blanking, which the new brace matching needs — `'{'` is a brace to anything counting depth. This also settles the question PR #44 handed here: experiment 2 in `docs/recomposition.md` was written as the one most likely to disagree with the analysis, on the reading that selecting a user invalidates the scope `MainNavScaffold` is called from. It does not — the read is inside the `content` lambda, and a composable lambda is compiled into a `ComposableLambda` whose `invoke` opens a restart group of its own, the same mechanism that makes `Scaffold`'s slots useful — so the rail is not on the selection path at all; a window-metrics update is what puts it on one. None of the six Gradle gates ran locally (`dl.google.com` answers 403 on CONNECT), so what was verified before pushing was the offline harness — 15 modules, 0 boundary violations, detekt 0 findings, `:app` tests up from 4 files/11 tests to 6 files/16 — plus a mutation test of both new rules, each made to fail with the intended message before being trusted. All four checks green first time, which is `compileDebugKotlin`, `lintDebug`, the Compose-metrics gate and `assembleDebug` confirmed. Not done: nothing was measured on a device, so the counts in both docs remain procedure rather than data; the contract test reads source, so an alias defeats it; and deferring reads behind a lambda parameter would cut more work on `HomeScreen` than this item does — the Scaffold body is still invalidated per keystroke because `SearchBar` needs the query — which is a different technique and its own decision (PR #46)
- [x] `LazyColumn` performance: keys, `contentType`, and item prefetch — two changes, two deliberate non-changes, and a third technique that turned out not to be a `LazyColumn` setting at all. `TextDetectedContent` put seven shapes in one list at the default `contentType` of `null`: a lazy layout offers a scrolled-off item's composition to an incoming item **only** when the two declare the same content type, because a slot table is already shaped like the tree that wrote it, so one type shared by a heading, a full-text panel, a run of block cards and two buttons means every reuse is a miss. It is the silent kind of wrong — every item renders, in order, at the right size, and the only symptom is a pool that never hits — which is why it survived four Compose-performance items before this one. `TextDetectedItem` names the shapes and puts the block cards, the only slot there is ever more than one of and therefore the only one reuse can pay off on, in a pool of their own. The same six slots were also unkeyed, and that half is a correctness bug rather than a missed optimisation: two of the seven shapes are emitted conditionally, so with no blocks detected the copy button, the scan-again button and the bottom spacer shift up by `blocks.size + 1`, and under index identity the runtime reads that as three items being replaced — remembered state dropped, an anchored scroll moved. An `enum` rather than string constants for two reasons that are both about failure: a key is written to a `Bundle` for saved item state and Compose's registry accepts a `Serializable`, which an enum entry is; and a typo in a string key is a silent duplicate, which the lazy layout throws on. **`HomeUserList` was left alone**, and the rule is shaped around that rather than around the finding: it emits one shape, so every item already shares one content type and naming it would partition the pool into the single class it already is — `LazyListContractTest` therefore requires a content type only of a list emitting more than one kind of slot, because a rule whose fix is pointless is one people learn to satisfy rather than read. The block cards stay index-keyed for the mirror-image reason: `RecognizedTextBlock` is a string and a float, two blocks in one frame can hold the same text, so a content-derived key repeats and a repeated key is an exception rather than a mis-render — and the list is only ever replaced whole, which is precisely the case index identity is correct for. That is a claim about the model and not about the source, so it is pinned (`EXPECTED_INDEX_KEYED`) rather than checked. **Item prefetch is two mechanisms and only one of them is in the UI**: Compose's lazy-layout prefetch composes one item beyond the viewport out of data it already has and cannot fetch, so whether a reader ever waits at the bottom of a paged list is decided by `PagingConfig.prefetchDistance` in `PagedUserRepositoryImpl`. It is now written out as `PAGE_SIZE` instead of left defaulted — the default *is* `pageSize`, so halving the page size to make requests cheaper would silently halve the prefetch window, a scroll-smoothness regression arriving out of a line that says nothing about scrolling. Behaviour unchanged; the coupling is now visible. Five contract rules, each mutation-tested to fail with its intended message and line before being trusted — including the one that closes the vocabulary's own hole, since a container spelling the test does not recognise makes its slots orphans and fails the pin rather than quietly going unaudited. `closingBracketAt` moved into `SourceTree` when this became its second caller, for the reason `KotlinSource` moved there in PR #46. None of the six Gradle gates ran locally — `dl.google.com` answers 403 on CONNECT from the agent's environment — so what was verified before pushing was the offline harness (15 modules, 0 boundary violations, detekt 0 findings, `:app` 16 tests → 21), the six mutations, and a parse of the three edited files the harness cannot compile: 613 diagnostics, every one a resolution or stdlib error and none syntactic. All four checks green on the first run. **Not done**: nothing was measured on a device, so `prefetchDistance` was made explicit rather than tuned and the reasoning here is about mechanisms rather than frame times; the contract test reads source, so an alias defeats it; and no screen consumes the paged stream yet — `docs/paging.md` expected that wiring to arrive here and it cannot, because search has to move into the DAO and the refresh fan-out loses its source of visible ids, which are data-layer contract decisions and are now their own Phase 9 item (PR #47)
- [x] Custom layouts and `SubcomposeLayout` for a measure-dependent component — two components in `:core:ui`, deliberately one of each kind, because the line between them is the item and the reflex gets it wrong in one direction. `LabelledValue` is a plain `Layout`: a caption beside its value, stacked when they do not fit, and genuinely measure-dependent — the same card is drawn across a phone by `ProfileScreen` and inside the detail half of a pane split by `ProfileDetailPane`, so how much room the two have is what is left of the window after a rail, a split and two lots of padding, for a value that may be twelve characters or sixty. No size class can answer that. What it is *not* is a reason to subcompose: both children exist under either outcome, so the measurement only chooses coordinates, and `Layout(contents = …)` gives the policy one list of measurables per lambda without composing anything itself — which was the most common honest reason to reach for `SubcomposeLayout` before that overload existed. The fit comes from `maxIntrinsicWidth`, because a `Measurable` may be measured once per pass so "measure it and see" is not on offer; the cost is a pass over the child's subtree, and the sharp end is that a lazy list cannot answer an intrinsic query at all, so asking for one constrains what may be put inside. **`ExpandableText` is the other kind.** Whether a "Show all" control exists is not a property of the string — a hundred characters is four lines on a phone and two in a dialog, one at `bodySmall` and three at `bodyLarge`, and more again at a larger system font — so only a text layout at this instance's actual width knows, which is the measure pass, after composition has decided what exists. That is the circle, and `subcompose` from inside the measure lambda is what breaks it. The version everyone writes first — `onTextLayout` assigning `hasVisualOverflow` into state — is a snapshot write *from* the layout phase: the first frame has no control and the second does, which is invisible on a screen composed once and not invisible in a `LazyColumn` item re-composed on every scroll back into view, and is also how an invalidation loop starts once the flag changes what is measured. Four slots, each under a distinct enum entry, and the `OverflowProbe` is the one worth knowing about: the same text at one line more than the collapsed limit, **measured and never placed**, which bounds the extra layout at one line rather than at the length of the text, and which is invisible rather than hidden — an unplaced node is not drawn and is not walked by semantics, so it reaches neither TalkBack nor a test's node tree. Both layouts' decisions are functions over pixels in `MeasurePolicy.kt` rather than lines inside a measure lambda, which is the move that puts them under a gate at all: a `MeasurePolicy` runs in the layout phase, so covering one in place means composing, which means `androidTest`, which still runs nowhere here. Two of the eleven cases are ones a device would otherwise find months later — `Constraints.Infinity` **is** `Int.MAX_VALUE` and a `Text`'s max intrinsic width is its unwrapped width, so three of them added as `Int`s wrap negative, the fit inverts, and the next line measures a child against a negative width; and `expanded` is screen state that outlives the string it was set for, so the plan derives both the body and the control from the measurement rather than obeying the caller, or a four-word scan inherits "Show less" from the page before it. `CustomLayoutContractTest` holds the three rules whose broken forms compile and render — a slot id that is not an enum entry, two slots sharing an id in one pass, and a layout that swallows its `modifier` — each mutated and made to fail with its intended message before being trusted, which is how the third was found too weak: a bare `\bmodifier\b` passes `SubcomposeLayout(modifier = Modifier)`, the mistake in its most plausible form, so it now requires the name to be used as a value and not as an argument label. **Two things outside the item that were blocking it.** CI had been failing at `Set up Android SDK` on every run of both workflows, whatever the commit: `android-actions/setup-android@v3` defaults `packages` to `tools platform-tools`, and `tools` is the obsolete package Google removed from the SDK repository, so `sdkmanager tools` exits 1 — fatal in `dependency-resolution.yml`, and in `ci.yml` a red job around a set of gates that all passed. `checkModuleDependencies` was also the one gate in that job without the `if: ${{ !cancelled() }}` its own comment promises every gate has, so the SDK failure *skipped* the layering gate while every gate below it still ran. And the offline harness: a module's `test` compilation now gets its own `main` output as a friend path, the association Gradle and the IDE make — without it `SearchPatternTest` could not see `internal fun likePattern` and, since a module producing no classes is a `sys.exit`, it had been taking `:data`'s whole test compilation, `:app`'s contract tests and detekt down with it while passing in CI the entire time; `UnidirectionalDataFlowContractTest` joins `HARD_EXCLUDES` because `HomeViewModel` stopped compiling there when it grew an `androidx.paging` import in PR #48, so it reported the harness's coverage as a missing view model. None of the six Gradle gates ran locally — `dl.google.com` answers 403 on CONNECT — so what was verified before pushing was the harness (15 modules, 0 boundary violations, detekt 0 findings, 316 tests → 334), the three mutations, and a parse of the seven files the harness cannot compile: 905 diagnostics, all resolution errors downstream of the missing classpath and none syntactic. Every check green. **Not done**: nothing was measured on a device, so the claims about frames are read off the phase model rather than a trace; `LabelledValue` asks for intrinsics and does not answer a parent asking for its own; `ExpandableText` does not animate, because an animating height is a height the overflow probe must not be compared against; and the contract test reads source, so an alias defeats it and it cannot follow a slot id through a helper — which is what the enum closes (PR #49)
- [x] Shared-element transitions + predictive back gesture — two elements between the home row and the profile, keyed on the user id, and both halves of this item fail the same way: they compile, they render, and the animation silently does not play, which is indistinguishable from the app as it was before anyone tried to animate it. **The key is a type because the agreement is cross-module.** A shared element is matched by `equals` and `rememberSharedContentState` takes `Any`, while the two halves are written in modules that may not import each other — the row in `:feature:home`, the profile in `:feature:profile` — so with a string the agreement is unchecked and a drifted spelling produces no transition, no exception and no log. `SharedElementKey` is a sealed interface of data classes with the id as a constructor parameter, because the key has to be unique per *user* and not per screen: forty rows under one `UserAvatar` is a duplicate key inside one `SharedTransitionScope`, which has no defined winner. The three raw names are confined to one file and asserted to be. **`sharedElement` vs `sharedBounds` is not a question of which looks better.** `sharedElement` animates one drawing between two rectangles and cross-fades nothing, so it is correct only when the two sides are *literally the same drawing* — which is why `UserMonogram` moved into `:core:ui` and both features call it, at 40dp and 80dp with the glyph sized as a fraction of the circle. Two per-feature copies would satisfy that precondition on the day they were written and stop satisfying it the first time a colour changed on one side, and the symptom would be a transition that jumps at its midpoint, on a device, months later; moving the widget is therefore what licenses the modifier rather than tidying. The display name is the same string at `titleMedium` and `headlineSmall`, so it is *not* one drawing and takes `sharedBounds` — the box travels, the contents cross-fade — and the email is not shared at all, because the profile has no comparable place for it and an element with no counterpart correctly does not animate. **The two scopes travel as a parameter rather than a `CompositionLocal`**, and that is the decision the item turns on: whether a screen takes part in a transition is a fact about where it is *drawn*, not about the screen. `HomeScreen` is the source half full-width in the nav graph and is not inside `HomeTwoPaneScreen`, where the row and the profile are visible simultaneously so both halves of every key would be live at once with nothing arriving or leaving to animate. An ambient would have given whichever branch sat inside the provider shared elements silently; as a parameter, `AppNavHost` states it once per branch in the one file that sees both, and the screens hold no `if` about an animation because the modifier helpers absorb the `null`. **Predictive back turned out to be one manifest attribute and one real bug.** `NavHost` already seeks its own pop transition from the gesture and a shared element follows that seek for free — but only through `OnBackInvokedDispatcher`, which is the only thing that reports *progress*, so without `android:enableOnBackInvokedCallback` every back still navigates one frame at a time and every animation above is dead. It is the highest-leverage line in the change and the easiest to lose to an unrelated manifest edit, so a test asserts it with XML comments stripped first — the mutation that removed the attribute and left the comment still failed. The bug is the list-detail layout: both panes live in one destination, so selecting a user is state, `NavHost` has nothing to pop, and back with a profile on screen left Home entirely. `BackHandler` fixes that and suppresses the system's predictive animation for as long as it is enabled, giving no preview and no way to abandon the gesture — the thing predictive back exists to fix — so `rememberPredictiveBackDismiss` is the same interception with the progress handed back. It separates commit from cancel without catching anything (a cancellation propagates, a `finally` winds the pane back, `onDismissRequest` sits after the `try`, and on commit the progress is left where the gesture ended so no frame of a full-size pane escapes), and its transform is a `graphicsLayer` **block**, so a gesture invalidates drawing rather than recomposing a detail pane and its view model's state every frame — the same reasoning `docs/derived-state.md` applies to `useListDetailLayout()`. `BackHandler` is now banned repository-wide. `SharedElementContractTest` holds five rules whose broken forms compile and render, each mutated and made to fail with its intended message before being trusted, plus a pin of the two keys and the files each is written in — deliberately without line numbers, since a pin that has to be re-pinned by every edit above it is a pin people stop reading. **A harness bug found on the way**: `Compiler.parse_only` filtered diagnostics with `startswith(("e: ", "error:"))`, which matches the *old* Kotlin CLI format, while Kotlin 2.x leads with the path — so it returned an empty list for any input at all, and its only caller, the `build-logic` parse gate, had been reporting "6 files parse" on every run without reading a single diagnostic. Found by parsing files that import `androidx` and getting back zero errors; both formats are accepted now and the fix was mutation-tested against a deliberate syntax error before being relied on. None of the six Gradle gates ran locally — `dl.google.com` answers 403 on CONNECT — so what was verified before pushing was the harness (15 modules, 0 boundary violations, detekt 0 findings, `:app` tests 16 → 22), the five mutations, and a parse of the nine files the harness cannot compile: 849 diagnostics, every one a resolution error against the missing androidx classpath and none syntactic. Every check green on the first run, both workflows. **Not done**: nothing was seen or measured on a device, so every claim about how the animation *looks* is read off the API contract — the `sharedBounds` enter/exit transitions, the `boundsTransform` and `resizeMode` are left at library defaults rather than chosen, and the dismiss scale and alpha are matched to the system's documented preview rather than tuned against it; nothing asserts at runtime that a transition ran, which needs an animation clock in an instrumented test and `androidTest` still executes nowhere here; `UserMonogram` does not scale with the system font (sized in `sp` off a `dp` container so the glyph cannot overflow a fixed circle) and still announces its letter to TalkBack, both unchanged from the copies it replaced and both belonging to the accessibility item next in this phase; and the contract test reads source, so an alias defeats it (PR #50)
- [x] Material3 dynamic colour, edge-to-edge, and full accessibility semantics — dynamic colour was already in `BoilerplateTheme` and `enableEdgeToEdge()` was already called, so two thirds of this item existed on paper. What that hid was a real bug. **`enableEdgeToEdge()` with no arguments answers the wrong question.** It gives both bars a `SystemBarStyle.auto` whose `detectDarkMode` reads `Configuration.UI_MODE_NIGHT_MASK` — the *device's* night mode — and this app also has a `ThemeMode` preference, so the two disagree for every reader who has chosen Light on a device in dark mode or Dark on one in light mode. The result is white status-bar icons over a white app bar: no clock, no battery, no signal, on the first screen such a reader sees. It compiles, it lints, and a screenshot taken in the matching configuration looks right — the same silent-in-one-configuration shape as the transition that does not play in PR #50. The fix is one `when`, moved onto the enum as `ThemeMode.isDark(systemInDarkTheme)` and placed in `:core:common` because `:app` and `:core:ui` both have to agree about it and neither may depend on the other: the theme picks its colour scheme with it, `MainActivity` builds both `SystemBarStyle`s' `detectDarkMode` from it. In a `DisposableEffect` keyed on the resolved answer rather than at `onCreate`, because the preference can change while the activity is resumed; the bare bootstrap call stays before `setContent`, or the first frame is drawn inside the bars, and the contract test below is written to permit exactly that one. **The semantics half is four things that were silent rather than wrong**, which is why no gate had caught any of them. The camera flash control announced what the *next tap* would do and never what the flash was doing — as an `IconButton` with a flipping `contentDescription` there is no state in the tree to read back, nothing to announce when it changes under a finger still on the control, and nothing for a reader arriving a second time. `IconToggleButton` puts it on `Modifier.toggleable`, so the role and the checked state are the platform's to phrase, with `stateDescription` supplying “On”/“Off” in place of the checkbox vocabulary; the icon now shows the state rather than the action, because a control announcing “Flash, On” beside a struck-through flash contradicts itself. It lives in `:core:ui` as `FlashToggle` for the reason `UserMonogram` does — `:feature:scanner` and `:feature:textrecognition` are siblings, each had a copy, and the copies had already diverged in everything but the event they raised. An indeterminate `CircularProgressIndicator` carries no semantics at all (`progressSemantics` is the determinate overload), so `LoadingIndicator` takes a `contentDescription` and the home refresh spinner names itself; both it and `OfflineBanner` are `LiveRegionMode.Polite`, because each appears without the user acting and nothing moves focus there. `TopAppBar` draws its title with a type style and no semantics, so every screen title is now `heading()` — swipe-by-heading had nothing to land on. And `UserMonogram` is `clearAndSetSemantics {}`: it draws the initial of a name the label beside it reads in full, which PR #50 left open here deliberately. **`AccessibilityContractTest` holds the two properties no other gate can see.** A `Scaffold` computes the insets its content must avoid and applies none of them — it hands the content lambda a `PaddingValues` — and Kotlin does not warn about an unused lambda parameter, so dropping it compiles, renders, and looks correct on whichever device happens to be open while drawing under the status bar, the navigation bar or a cutout elsewhere. The second rule is the bug above, and it is per *file* rather than per call for the bootstrap reason. Both read source for the reason `LazyListContractTest` does: each property is about an argument, and no class file records one. Two discovery pins stop either rule from passing by finding nothing, and a four-case fixture proves the padding detector can actually report — the pins count scaffolds, not verdicts, so a detector that silently matched nothing would leave them green. The one name the rule cannot see through is a lambda parameter called `padding`, which its own body's `Modifier.padding(` would match; written down beside the fixture. **Verified**: none of the six Gradle gates ran locally — `dl.google.com` still answers 403 on CONNECT — so the offline harness is what ran before pushing (15 modules, 0 boundary violations, detekt 0 findings over main, test and androidTest, `:app` 22 tests → 27 and `:core:common` +4), plus a parse of every changed file including the nine the harness cannot compile: 0 syntactic diagnostics. CI is therefore the first thing to have type-checked `MainActivity`, `FlashToggle`, `LoadingIndicator`, `UserMonogram`, `BoilerplateTheme` and the four changed screens, and it settled the two questions a local compile would have: `SystemBarStyle.auto`'s parameter names used as named arguments, and `IconToggleButton` accepting a `semantics` modifier alongside its internal `toggleable`. Every check green on the first run of both workflows — `checkModuleDependencies`, `compileDebugKotlin`, the Compose-metrics gate (the new composable is skippable, the allowlist stays empty), Room schemas, `lintDebug`, `detekt`, `testDebugUnitTest`, version resolution and the verified APK. **Not done**: `UserMonogram`'s glyph still does not scale with the system font — sized in `sp` off a `dp` container so a letter cannot overflow a fixed circle, and making the circle itself scalable is open; nothing enforces touch-target size for a control that is not a Material component; contrast under dynamic colour is the system's to guarantee and is checked by nothing here, which is why `docs/accessibility.md` states the no-hardcoded-colours rule rather than gating it; `FlashToggleTest` is detekt-checked and otherwise unrun, because `androidTest` executes nowhere — neither here nor in CI, which has no emulator job — so an `AccessibilityChecks`-enabled instrumentation pass remains the obvious next gate; and the contract test reads source, so an alias for `Scaffold` or a wrapper around `enableEdgeToEdge` defeats it (PR #51)

## Phase 11 — Security & Release
- [x] EncryptedSharedPreferences / Keystore-backed token storage, never plaintext — AES-256-GCM over an `AndroidKeyStore` key, and **not** `EncryptedSharedPreferences`, which this item names first: `androidx.security:security-crypto` is deprecated rather than promoted past alpha, and adopting it would mean a second storage mechanism beside the `Preferences` `DataStore` the tokens already live in — `SharedPreferences` semantics, a second threading model, and `tokensFlow` rebuilt on a change listener. What it does internally is Tink over a keystore-held master key; what lands here is the same construction with the parts named in the repository and no new dependency. **The IV is never chosen in this code.** `Cipher.init` generates it and it is read back off the cipher, which is both the correct JCA idiom and the only thing that works against a key built `setRandomizedEncryptionRequired(true)` — so an edit that helpfully passes a caller-supplied `GCMParameterSpec` fails loudly on a device instead of reusing an IV, and a reused IV under GCM leaks the authentication key itself, not merely a plaintext. **The half with no symptom was backup.** `android:allowBackup` is true and Auto Backup copies `filesDir`; the keystore key does not go with it, which is most of what a keystore is for, so a restore lands well-formed ciphertext on a device that can never read it — nothing throws, and the reader is simply signed out on their new phone with nothing in any log. Both `backup_rules.xml` and `data_extraction_rules.xml` exclude the store, because the platform reads `fullBackupContent` below API 31 and `dataExtractionRules` from 31 up and never both, so naming one covers half an install base whose `minSdk` here is 26; `device-transfer` is excluded beside `cloud-backup` because a handset-to-handset copy has no cloud in it and carries the key no better. **Decryption failure is a state rather than an error**: `decrypt` answers `null` for a bad tag, an invalidated key, cleared app data, a value that is not Base64 or one too short to be a message, and `StoredTokens.read` turns that into `Discard` — clear the store, nobody signed in. A keystore unreachable *at all* is a `ProviderException` and propagates, failing the request that asked, because mapping that to `null` would spend a session on a transient hardware fault silently. Plaintext stores from earlier versions are adopted on first read, re-encrypted and deleted rather than signed out: anything that could read the file already has the token and clearing the app's copy does not revoke it, so a forced sign-out costs every reader their session for a threat that has already either happened or not. `AuthTokens.toString` is redacted — a `data class` prints its properties, so one interpolation put both credentials in a log or a crash report, which encrypting the store does nothing about. **One harness change, and it is a widening**: `javax.crypto` was missing from the resolvable-imports pattern, which spelled `javax` out only for `javax.inject` — a fetched jar, where `javax.crypto` is `java.base` and needs no artifact — and that gap was skipping the cipher and its test as though they needed the SDK. With it added, 25 tests run locally that otherwise would not, covering where the risk actually sits: AES is AES, but which combination of present, absent and undecryptable means signed in, signed out or clear the store is a table, so it is a pure function over a data class rather than four reads inside the provider. `TokenStorageContractTest` pins that the two legacy plaintext keys are only ever read and removed — an assignment to one puts a readable credential back on disk and no other test would notice, because the app reads the encrypted pair — and both of its XML rules were mutation-tested before being trusted. **Verified**: none of the six Gradle gates ran locally, `dl.google.com` still answering 403 on CONNECT, so the offline harness is what ran before pushing (113 of 180 main files, 55 of 85 test files, detekt 0 findings, skip list otherwise byte-identical to main's) plus a parse of the five files it cannot compile — 0 syntactic diagnostics against 381 resolution errors from the absent classpath. CI is therefore the first thing to have type-checked `AndroidKeystoreSecretKeySource`'s `KeyGenParameterSpec` builder chain, the two Hilt `@Binds`, and the DataStore provider with its six new tests; every check green on the first run of both workflows. **Not done**: no `setUserAuthenticationRequired` on this key, deliberately — `UserSyncWorker` runs six-hourly and signs with this token, so biometric gating would turn every background sync on a locked device into a silent failure, and it belongs on a step-up secret with a foreground prompt to attach it to; nothing ran on a device, so "the keystore hands back a usable AES key" is read off the API contract because `androidTest` still executes nowhere; and the contract test reads source and XML, so an alias defeats it (PR #52)
- [x] Certificate pinning in OkHttp with a documented rotation plan — pinning is the security feature with no positive signal. An app that pins correctly and an app that pins nothing behave identically — same requests, same responses, same green suite — until the day somebody is between the device and the server, and on that day only one of them notices. So almost none of this item is `.certificatePinner(…)` and almost all of it is turning the ways pinning silently stops protecting anything into things that fail loudly. **A host with no pins is not an error in OkHttp**: `check` looks the hostname up, finds nothing and returns, so a typo in a hostname or a `*.` where `**.` was meant turns pinning off in full while the client still carries a pinner, the build passes and every request succeeds. `PinningPolicy.decideFor` compares the pin set against `BuildConfig.BASE_URL`'s host and throws when they do not meet — at the first injection of an `OkHttpClient`, which is app start, so a build in that state cannot get past a developer machine. **A pin set with one pin is one key's lifetime**: the day that key has to go, every installed copy stops reaching the server and no server-side change can fix it, because the app is what holds the pin — two distinct pins per host is a construction-time requirement and the second is a key generated, kept offline and not yet serving traffic. **A pin set is a time bomb with no timer**, so the expiry is mandatory and past it enforcement stops rather than bricking installations too old to know the current keys, which is what Android's own `<pin-set expiration>` does and for the same reason: refusing to connect there has no recovery path, since an in-app update prompt needs the network too. Misconfiguration fails closed and expiry fails open; they are different failures and the difference is who can fix them. `covers()` delegates wildcard matching to `CertificatePinner.findMatchingPins` rather than reimplementing it — a second implementation of those semantics would agree with the real one in every test anybody thought to write and disagree in production. **Both clients are pinned, and the auth one is where forgetting it costs most**: it carries the password on sign-in and the refresh token on every rotation, the two credentials the other never sends, and it is the easy one to miss because it shares no configuration with the main client. The pin set itself is **empty**, which is the only honest default — the pins would have to name the keys of a real server and this repo points at `api.example.com`, and wrong pins are strictly worse than none — so `:data:compileReleaseKotlin` fails while `gradle/certificate-pins.properties` declares nothing, and the build validates hosts and pins by regex and re-serialises the expiry from a parsed `Instant` before interpolating, because `buildConfigField` takes *source text* and an unvalidated value is an injection into generated code. `isTransientFailure` also changed: it treated every `IOException` as retryable and `SSLPeerUnverifiedException` is one, while being the least transient failure there is — the next attempt gets the same chain and the same verdict — so it was spending every layer's backoff budget and reading downstream as a flaky connection, which is the one wrong thing to say about the one failure that means somebody may be in the middle of the connection; narrowed to that class and not its parent `SSLException`, which is also what a handshake killed by a dropped connection arrives as. **The strongest test is the one that touches a socket.** `CertificatePinningHandshakeTest` signs a CA and two leaf certificates in-process, puts MockWebServer behind TLS and drives real requests with the client *trusting* that CA — so system trust is satisfied and anything rejected is rejected by the pins alone: a matching pin connects, a pin set naming neither serving key throws, a backup pin carries a server that has already rotated to the second key (the rotation plan, executed), and an expired set connects anyway. `okhttp-tls` is ordinary `java.security`, so all five run in the offline harness too. `CertificatePinningContractTest` holds the three source-level rules — every `OkHttpClient.Builder()` chain gets a pinner, `CertificatePinner.Builder()` exists in exactly one file, and `sslSocketFactory`/`hostnameVerifier` appear nowhere in `src/main` — each with a fixture proving the detector can still report, the chain scanner pinned against both ways it could go quiet. **CI went red once and the failure is worth recording**: eleven Kotlin script compilation errors in `data/build.gradle.kts`, all one mistake — in a build script `java` resolves to the `JavaPluginExtension` accessor the Java plugin contributes, not to the package root, so `java.util.Properties`, `java.io.StringReader` and `java.time.Instant` were property accesses on that extension and the other eight errors were inference failures downstream of `properties` becoming an error type. The deeper problem was that nothing local had ever compiled that code: Gradle cannot run here and the harness *parses* build scripts without type-checking them. Fixed with imports above `plugins { }`, and verified by extracting the exact shipped text of the two regexes and `readCertificatePins`, shimming the three Gradle symbols it names, and compiling and running it under the same `K2JVMCompiler` the harness uses — the checked-in file parses to an empty pin set, a configured one serialises with hosts sorted, and six malformed inputs are rejected including a host carrying a quote and a pin carrying `";x="`. A standing gate for that is genuinely wanted and deliberately not in this change: type-checking a Gradle script needs the Gradle API and AGP on the classpath, which this environment cannot fetch, so any local check has to extract a region by hand — its own piece of work rather than something to bolt onto a red PR. `javax.net` joined the harness's resolvable-import pattern beside `javax.inject` and `javax.crypto`, the same gap found the same way as in PR #52: without it, naming `SSLPeerUnverifiedException` would have dropped `FlowRetry.kt` and its 122 tests out of the run. **Verified**: none of the six Gradle gates ran locally — `dl.google.com` still answers 403 on CONNECT — so the harness is what ran before each push (15 modules, 0 boundary violations, 114 of 181 main and 58 of 88 test files, 383 tests → 413, detekt 0 findings over main, test and androidTest, skip list byte-identical to main's), plus a parse of `NetworkModule.kt`, which the harness cannot compile: 109 diagnostics, all resolution, none syntactic. CI is therefore the first thing to have type-checked `NetworkModule`'s two providers and the whole build script; every check green on the second run of both workflows — `checkModuleDependencies`, `compileDebugKotlin`, Compose metrics, Room schemas, `lintDebug`, `detekt`, `testDebugUnitTest`, version resolution and the verified APK. **Not done**: the release gate's task wiring is the one thing still unverified anywhere — CI assembles debug only, so `tasks.matching { it.name == "compileReleaseKotlin" }` is read off AGP's task names rather than observed, and it is written with `matching` rather than `named` so a wrong name makes it inert rather than breaking every build; nothing ran on a device, so the handshake test exercises OkHttp's pinner and not the platform's TLS stack; no intermediate- or root-CA pinning and no remote pin delivery, both argued in `docs/certificate-pinning.md`; and the contract test reads source, so an alias or a wrapper defeats it (PR #53)
- [x] Root/tamper detection with Play Integrity API — the item's obvious reading is the one thing this deliberately does not do. A `su` on the path, `test-keys` in `Build.TAGS`, a root manager's package name: every one of those checks runs on the device the attacker owns, and an `if` on a device the attacker owns is an `if` the attacker removes. What it costs is a permanent stream of false positives from people with a custom ROM or an odd OEM build; what it buys is a few hours of one attacker's time, once. So there is no `isRooted()` and `IntegrityAttestationContractTest` fails the build if somebody adds one — along with any client-side read of a verdict, because a `MEETS_DEVICE_INTEGRITY` branch in the app is the same mistake wearing the new API's clothes. What the app does instead is plumbing for a verdict computed where the attacker cannot reach it: obtain a token, bind it to the request, attach it, let the backend decode and decide. Three parts carry the weight. `IntegrityRequestHash` is the binding — without it a token attests a device-moment rather than a request, and anyone holding one can attach it to any call — so its canonical form is written out as a wire contract with golden vectors a server-side implementation can be checked against. `IntegrityInterceptor` is opt-in per endpoint, because tokens come from a per-device budget and `AuthApi.login` is where one buys the most; it fails open, because refusing to send is a client-side security decision — the category this API exists because the client cannot make — and it would lock out only the honest population. `PlayIntegrityErrorCode` writes Play's seventeen codes out as numbers so the mapping compiles and is tested without the Android toolchain, with a reflection contract test holding it to the real library, which also turns a code added by a future release into a red test rather than a silent `else`. Every `OkHttpClient` carries the interceptor rather than only the one with a marked endpoint: a marked endpoint on a client without it sends the marker header to the server and no token, which is indistinguishable from a device that could not attest. Attestation is off — `gradle/play-integrity.properties` is empty, and a fabricated project number would fail with `CLOUD_PROJECT_NUMBER_IS_INVALID` on every device rather than degrade — with a release gate mirroring the pinning one. Also here because the same one line covers both: `Authorization` and `X-Integrity-Token` are redacted from request logging, which at `BODY` level was printing a live bearer token. `docs/root-detection.md` has the threat model, the server's half, and the rollout order that avoids rejecting everyone at once. Not done: nothing ran on a device, the token says nothing about the session it opens, and the standard API's verdict is thinner than the classic one's
- [x] R8 full mode with keep rules + a mapping-file upload step — the keep rules were the problem and the shrinker was never the problem. `app/proguard-rules.pro` was a file no build in this repository read: R8 only runs on a minified variant, the only one was `release`, and `release` is deliberately unbuildable while the certificate pins and the Play Integrity project are empty. So a rule that matched nothing, a rule that was missing and a rule that worked all produced the same green CI — and R8 warns about none of them, because a `-keep` whose pattern names no class is legal. **Two of the six rules had been inert for the life of the file**: one kept `com.kojo.boilerplate.navigation.**`, a package this repository has never had, and one asked for a `Parcelable` `CREATOR` field on `**$$serializer` classes, which have none. What replaces them is three rules, each checked against the artifact that needs it rather than against a memory of its README — Maven Central is reachable here even though Google Maven is not, so `retrofit2.pro`, `okhttp3.pro` and `kotlinx-serialization-*.pro` were read directly and none of their rules is duplicated. The app owns exactly what a library rule cannot name: `INSTANCE` and `serializer()` on its own `@Serializable` types, because the four `data object` routes have an `INSTANCE` no code reads and the conditional rule kotlinx.serialization 1.7.3 ships does not hold that up in full mode (the unconditional fix is upstream in 1.8.0, so the rule is deletable on that bump); `<fields>` on `GeneratedMessageLite` subclasses, because `protobuf-javalite` is the one dependency here that ships no consumer rules at all and `MessageSchema` resolves field *names* with `getDeclaredField`; and the two line-number attributes, without which a retraced frame has no line. **The Hilt rule was deleted on evidence rather than on suspicion**: `hiltViewModelKeys` in 2.57.2 is a `Map<Class<?>, Boolean>` keyed by class literals, read off the artifact's own bytecode with `javap`, so nothing reads a view model's name and `-keepnames` was costing every one of them its obfuscation for a lookup that no longer exists. Giving R8 somewhere to run is the `minified` build type — `initWith(release)` so the two configurations cannot drift, the libraries' `debug` halves via `matchingFallbacks`, debug signing — in the CI job that already ran `assembleDebug`, which makes R8 the only new work and cost 4½ minutes. Putting placeholder pins into CI to build the real `release` instead would have defeated the gate whose whole point is that nobody can drift past it. **The rules are gated by their effect**: `scripts/verify-r8-mapping.py` fails when a `@Serializable` type or generated message did not survive, when `INSTANCE`, `serializer`, `Companion` or a protobuf `_` field was renamed, when nothing under the app's package was obfuscated at all, when `missing_rules.txt` has anything in it, or when a line in class or member position went unread — and the sets it requires are derived from the Kotlin and `.proto` sources, not listed, because a list is what stopped covering the destinations when they moved. **The first run of it found three things, which is the whole argument for the build type.** The parser could not read a real mapping: Kotlin mangles a lambda capturing an inline-class value into `ClickableKt$clickable-O2vRcR0$$inlined$…` and a class line that fails to parse orphans every member under it, so twenty bad lines produced 2354 complaints; both patterns are `\S+`-based now and the real lines are a verbatim fixture. And three classes the sources say should survive were shrunk away, correctly: nothing calls `AuthApi.login`, so `LoginRequest` goes with the endpoint, and nothing injects `UserPreferencesDataSource`, so the entire Proto DataStore chain — module, serializer, data source, both messages — is unreachable, which also means the protobuf rule protects nothing until a screen reads a typed preference. Forcing them alive with a `-keep` would have shipped dead code to satisfy a gate, so instead they are named in `config/r8/shrunk-away-allowlist.txt` with their reasons, checked in both directions like the compose-metrics allowlist: an entry for a class that *is* in the output is stale and fails, and so is an entry the gate never asks about. The mapping, with the `usage.txt` and `seeds.txt` R8 does write for this variant, uploads as `r8-mapping-minified` for 90 days. `docs/r8.md` has the rules, the retrace procedure and the findings. Not done: no release mapping goes to Play — that belongs with the signing item below; `isShrinkResources` stays off; the three allowlisted classes stay unreachable, which is app work; and nothing has run on a device.
