 ...
 * ### What it reads
 *
 * The compiled output of `src/main`, walked from the directory this module's own classes were
 * loaded from — the same discovery `StabilityContractTest` uses, for the same reason: a
 * hand-written list of types omits the one added next month. Classes are loaded with
 * `initialize = false` so reaching one cannot run a static initialiser that touches the
 * Android framework, which is only a stub under a unit test. Types are matched by role
...
class SolidContractTest {
    ...
    private fun implementationsOf(abstraction: Class<*>): List<Class<*>> = appClasses()
        .filterNot { it.isInterface }
    ...
    private fun typesInRole(suffix: String): List<Class<*>> = appClasses()
    ...
    /**
     * Every class the Kotlin compiler produced for `src/main`, anchored on a main-source type
     * rather than on this test, whose classes sit in a different output directory.
     */
    private fun appClasses(): List<Class<*>> {
        val root = File(UserRepository::class.java.protectionDomain.codeSource.location.toURI())
        val loader = UserRepository::class.java.classLoader
    ...
    /** The same walk for the day AGP hands unit tests a packaged classpath instead of directories. */
